import sketch, { Document, Style } from 'sketch';
const base64Image =
...
async function getUserData() {
    ...
    const name = {
        ...
        last: data.name.last,
        full: `${data.name.first} ${data.name.last}`,
    };
...
export default function(context) {
    ...
    getUserData().then((user) => {
        // iterate over selected layer
        selection.forEach((layer) => {
            console.log(layer);
            // check if layer type is 'Text' and name is 'Name'
            if (layer.type === 'Text' && layer.name === 'Name') {
                // replace text string with name from data
                layer.text = user.name.full;
            }
            // check if layer type is ShapePath and name is 'Image'
            else if (layer.type === 'ShapePath' && layer.name === 'Image') {
                // change fill to picture from data
                layer.style.fills = [
                    {
                        fill: 'Pattern',
                        pattern: {
